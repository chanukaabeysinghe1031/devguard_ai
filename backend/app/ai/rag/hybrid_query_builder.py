...
class HybridDiagnosticQueryBuilder:
    ...
    def build(self, context: AnalysisContext) -> DiagnosticQuery:
        ...
        if isinstance(override, str) and override.strip() and isinstance(structured, dict):
            ...
            if isinstance(structured.get("exception_names"), list):
                query.exception_names = [str(x) for x in structured["exception_names"] if x][:8]
            ...
            if isinstance(structured.get("resource_types"), list):
                query.resource_types = [str(x) for x in structured["resource_types"] if x][:8]
